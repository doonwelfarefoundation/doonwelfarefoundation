...
function LanguageSelect({ onDark }: { onDark: boolean }) {
  ...
  return (
    <div className="relative inline-flex items-center">
      <svg
        className={`pointer-events-none absolute left-2.5 h-4 w-4 ${onDark ? 'text-white/80' : 'text-ink'}`}
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="1.8"
      ...
      <svg
        className={`pointer-events-none absolute right-2.5 h-3.5 w-3.5 ${onDark ? 'text-white/80' : 'text-ink'}`}
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
...
export default function Navbar() {
  ...
  // Home has a full-bleed dark hero, so the bar can start transparent there.
  // Inner pages use a dark PageHero banner too, so transparent-over-dark also holds.
  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-surface/90 shadow-md shadow-black/5 backdrop-blur' : 'bg-transparent'
      }`}
    >
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3">
        <Link href="/" className="flex flex-none items-center gap-3" aria-label={t.a11y.home}>
          ...
          >
            Doon Welfare
            <span className="block text-xs font-medium tracking-wide opacity-80">Foundation</span>
          </span>
        </Link>
      ...
      {open && (
        <div className="max-h-[80vh] overflow-y-auto border-t border-line bg-surface px-5 py-4 lg:hidden">
          <div className="flex flex-col gap-3">
            {links.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className={`text-sm font-medium ${isActive(l.href) ? 'text-brand-gold' : 'text-ink'}`}
              >
                {l.label}
              </Link>
            ))}
